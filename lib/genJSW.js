...
(function() {
  var BinaryPrecedence, CodeGenerator, E_FFT, E_FTT, E_TFF, E_TFT, E_TTF, E_TTT, FORMAT_DEFAULTS, FORMAT_MINIFY, F_ALLOW_CALL, F_ALLOW_IN, F_ALLOW_UNPARATH_NEW, F_DIRECTIVE_CTX, F_FUNC_BODY, F_SEMICOLON_OPT, Precedence, S_FFFF, S_TFFF, S_TFFT, S_TFTF, S_TTFF, SourceNode, Syntax, addComments, addIndent, adjustMultilineComment, base, calculateSpaces, createSrcNodesFile, crypto, directive, endsWithLineTerminator, escapeAllowedCharacter, escapeDirective, escapeDisallowedCharacter, escapeRegExpCharacter, escapeString, escapeless, estraverse, esutils, extra, flattenToString, fs, generate, generateAsyncPrefix, generateBlankLines, generateComment, generateIdentifier, generateInternal, generateMethodPrefix, generateNumber, generateRegExp, generateStarSuffix, generateVerbatim, generateVerbatimString, getDefaultOptions, hasLineTerminator, hexadecimal, indent, isArray, isExpression, isStatement, join, json, jswAdd, jswIndexes, jswPop, jswPush, jswStack, jswTop, log, merge, newline, noEmptySpace, origCode, originalSourceCode, parentheses, parenthesize, parse, preserveBlankLines, quotes, renumber, rimraf, safeConcatenation, semicolons, sourceCode, sourceMap, space, stringRepeat, toSourceNodeWhenNeeded, updateDeeply, withIndent;
  ...
  jswPush = function(nodeType) {
    ...
    return jswStack.push(jswTop);
  };

  jswAdd = function(node, src) {
    base = stringRepeat(' ', jswTop.depth - 1);
    jswTop.src += base + flattenToString(src);
    return src;
  };

  jswPop = function(pass) {
    var len;
    ...
    if (node.loc == null) {
      ...
      if (node && isStatement(node) && node.type !== 'Program') {
        if (createSrcNodesFile) {
          fs.appendFileSync('test/srcNodes.txt', '\n##### ' + node.type + ': \n  ~' + originalSourceCode.slice(node.start, node.end) + '~\n  ~' + srcStr + '~\n' + JSON.stringify(jswTop) + '\n');
        }
      }
      return srcNode;
    }
    srcNode = new SourceNode(node.loc.start.line, node.loc.start.column, (sourceMap === true ? node.loc.source || null : sourceMap), generated, node.name || null);
    node.jswSrc = srcStr;
    jswAdd(node, srcStr);
    return srcNode;
  };
  ...
  join = function(left, right) {
    ...
    return jswPop([left, space, right]);
  };
  ...
  generateVerbatim = function(expr, precedence) {
    ...
    return jswPop(toSourceNodeWhenNeeded(result, expr));
  };
  ...
  generateIdentifier = function(node) {
    ...
    return jswPop(toSourceNodeWhenNeeded(node.name, node));
  };
  ...
  CodeGenerator.prototype.maybeBlockSuffix = function(stmt, result) {
    ...
    return jswPop([result, newline, base]);
  };
  ...
  CodeGenerator.Statement = {
    ...
    ExportDeclaration: function(stmt, flags) {
      ...
      if (stmt.declaration) {
        return jswPop(join(result, this.generateStatement(stmt.declaration, bodyFlags)));
      }
      if (stmt.specifiers) {
    ...
    ImportDeclaration: function(stmt, flags) {
      ...
      that = this;
      if (stmt.specifiers.length === 0) {
        return jswPop(["import", space, this.generateExpression(stmt.source, Precedence.Sequence, E_TTT), this.semicolon(flags)]);
      }
      result = ["import"];
      cursor = 0;
    ...
    FunctionDeclaration: function(stmt, flags) {
      ...
      return jswPop([generateAsyncPrefix(stmt, true), "->", generateStarSuffix(stmt) || noEmptySpace(), generateIdentifier(stmt.id), this.generateFunctionBody(stmt)]);
    },
  ...
  CodeGenerator.Expression = {
    ...
    MemberExpression: function(expr, precedence, flags) {
      ...
      return jswPop(parenthesize(result, Precedence.Member, precedence));
    },
    UnaryExpression: function(expr, precedence, flags) {
      ...
      return jswPop(parenthesize(result, Precedence.Unary, precedence));
    },
    ...
    ObjectExpression: function(expr, precedence, flags) {
      ...
      if (!multiline && !hasLineTerminator(toSourceNodeWhenNeeded(fragment).toString())) {
  ...
  CodeGenerator.prototype.generateStatement = function(stmt, flags) {
    ...
    return jswPop(toSourceNodeWhenNeeded(result, stmt));
  };
